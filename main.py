...
from PATIENT_SERVICE_API.lib import patient_crud
...
# for physicians
@app.get("/physicians/", response_model=List[response_models.Patient])
def get_patients(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    patients = physician_crud.get_patients(db, skip=skip, limit=limit)
    return patients